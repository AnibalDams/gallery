...
class Media {
    ...
    async save(res: Response, userEmail: string) {
        try {
            const uploadImageToCloudinary = await cloudinary.v2.uploader.upload(this.path);
            const imageUrl = uploadImageToCloudinary.url;
            const public_cloudinaryId = uploadImageToCloudinary.public_id;

            const media = new MediaM({ url: imageUrl, type: this.type, size: this.size, cloudinaryId: public_cloudinaryId, userEmail: userEmail });
            await media.save();
            res.json({ data: media });
        } catch (e) {
            console.error(e);
